...
fn main() {
    ...
    println!(
        "  SSH Session:      {}",
        if env_info.is_ssh { "Yes" } else { "No" }
    );
    println!(
        "  Container:        {}",
        if env_info.is_container { "Yes" } else { "No" }
    );
    println!(
        "  tmux:             {}",
        if env_info.is_tmux { "Yes" } else { "No" }
    );
    println!(
        "  GNU screen:       {}",
        if env_info.is_screen { "Yes" } else { "No" }
    );
    println!("  Terminal Type:    {}", env_info.term_type);
    println!("  Color Support:    {:?}", env_info.color_support);
    println!(
        "  True Color:       {}",
        if env_info.has_truecolor { "Yes" } else { "No" }
    );
    println!(
        "  Mouse Support:    {}",
        if env_info.has_mouse_support {
            "Yes"
        } else {
            "No"
        }
    ...
    println!(
        "  Font Ligatures:   {}",
        if settings.enable_font_ligatures {
            "Enabled"
        } else {
            "Disabled"
        }
    ...
    println!(
        "COLORTERM:        {}",
        std::env::var("COLORTERM").unwrap_or_else(|_| "not set".to_string())
    );
    println!(
        "TERM_PROGRAM:     {}",
        std::env::var("TERM_PROGRAM").unwrap_or_else(|_| "not set".to_string())
    );
    println!(
        "SSH_CONNECTION:   {}",
        std::env::var("SSH_CONNECTION").unwrap_or_else(|_| "not set".to_string())
    );
    println!(
        "TMUX:             {}",
        std::env::var("TMUX").unwrap_or_else(|_| "not set".to_string())
    );
    println!(
        "STY:              {}",
        std::env::var("STY").unwrap_or_else(|_| "not set".to_string())
    );
    println!(
        "LANG:             {}",
        std::env::var("LANG").unwrap_or_else(|_| "not set".to_string())
    );
}
