...
//!
//! Run with: cargo run --example theme_editor_demo
...
fn demo_iterm2_export() {
    ...
    // let path = Path::new("/tmp/tokyo_night.itermcolors");
    // std::fs::write(path, iterm_xml).unwrap();
...
fn demo_vscode_export() {
    ...
    // Save to file (commented out to avoid creating files in demo)
    // let path = Path::new("/tmp/one_dark.json");
    // std::fs::write(path, vscode_json).unwrap();
    // println!("Saved to: {:?}", path);
...
mod tests {
    ...
    #[test]
    fn test_demo_runs() {
        // Just verify the demos can run without panicking
        demo_color_picker();
        demo_theme_preview();
        demo_theme_editor();
        demo_iterm2_export();
