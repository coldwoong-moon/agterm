...
use crate::error::TaskResult as TResult;
...
impl TaskScheduler {
    ...
    pub fn can_start_more(&self) -> bool {
        !self.paused && !self.cancelled && self.running.len() < self.config.max_concurrent
    }
    ...
    pub fn get_next_tasks(&mut self) -> Vec<(TaskId, String, Vec<String>)> {
        ...
        for task_id in ready.into_iter().take(available_slots) {
            ...
            if let Some(task) = self.graph.get_task(&task_id) {
                result.push((task_id, task.command.clone(), task.args.clone()));
    ...
    pub fn task_completed(
        ...
        if success {
            ...
            let policy = task.map_or(self.config.error_policy, |t| t.error_policy);
        ...
        for (task_id, command, args) in self.get_next_tasks() {
            events.push(SchedulerEvent::TaskReady {
                task_id,
                command,
                args,
            });
    ...
    pub async fn run(mut self) {
        ...
        while let Some(cmd) = self.command_rx.recv().await {
            let events = match cmd {
                ...
                SchedulerCommand::TaskCompleted { task_id, result } => self
                    .scheduler
                ...
                SchedulerCommand::CancelAll => self.scheduler.cancel_all(),
                ...
                SchedulerCommand::Resume => {
                    ...
                    self.scheduler
                        .get_next_tasks()
                        .into_iter()
                        .map(|(task_id, command, args)| SchedulerEvent::TaskReady {
                            task_id,
                            command,
                            args,
                        })
...
mod tests {
    ...
    fn test_task_lifecycle() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, SchedulerEvent::TaskReady { .. })));
    }
